...
class DiskRadialModel(object):
    ...
    def __init__(self, mstar=nc.MS, lstar=nc.LS, tstar=nc.TS, rin=0.1 * nc.au,
                 rout=200 * nc.au, nr=100, rgrid=None, mdisk=None, plsig=-1,
                 flang=0.05, alpha=1e-2, qtoomre=None, mdot=None, hpr=None,
                 meanopacitymodel=None, tbg=2.725):
        self.mstar = mstar
        self.lstar = lstar
        self.tstar = tstar
        ...
        if hpr is None:
            ...
            self.tmid = 2.3 * nc.mp * self.cs**2 / nc.kk
    ...
    def solve_viscous_heating_globally(self, ttol=1e-2, qtol=1e-4, nitermax=100,
                                       fixmeanopac=False, simple=False,
    ...
    def compute_onezone_intensity(self, lam, dust=None, inclbg=True, inclscat=True, inclang=0.):
        ...
        for d in dust:
            ...
            if inclscat:
                ...
                gsca      = np.interp(lammic, g.opac_lammic, g.opac_gsca)
            else:
                kapsca    = 0.
                gsca      = 0.
        ...
        for inu in range(nf):
            ...
            if not inclscat:
                ...
                def scatsolution_eddbarbier(tauabs,tauscaeff,bpl,bplbg,mu):
                    ...
                    ii       = tauobs < 1e-6
                    xpt[ii]  = tauobs[ii]
                    return xp * bplbg + xpt * source                # Eq.(18) plus background radiation
                self.intensity[inu,:] = scatsolution_eddbarbier(tauabs[inu,:],tauscaeff[inu,:],bpl,bplbg)
    ...
    def get_viscous_evolution_next_timestep(self, dt, alphamodel=True, sigma_innerbc=None, exttorque=None):
        """
    ...
    def compute_viscous_evolution_next_timestep(
            ...
            vischeat=False, sigma_innerbc=None):
        """
        ...
        and assure that mdot>0 at the inner edge:
          ...
          assert d.mdot[0]>=0., 'Problem: outward pointing flow at inner boundary!'
        """
        self.sigma = self.get_viscous_evolution_next_timestep(dt, alphamodel=alphamodel, sigma_innerbc=sigma_innerbc)
...
class ShuUlrichModel(object):
    ...
    def __init__(self,xmn=1e-6,nx=1000,mcloud=None,tcloud=None,omcloud=None,
        ...
        cs                  = np.sqrt(nc.kk*tcloud/(2.3*nc.mp))
