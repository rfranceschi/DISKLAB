...
class Disk2D(object):
    def __init__(self, disk, zrmax=None, nz=90, meanopacitymodel=None,
        ...
        # set flaring angle and flaring ndex depending on irradiation method
    ...
    def convert_1p1d_to_cyl2d(self,rhogas=False,tgas=False,rhodust=False,irrad=False,
        ...
        if hasattr(self.disk, 'dust'):
            ...
            rhodust = False
        if(rhogas):   self.cyl2d_rhogas  = np.zeros((nr,nz))
        if(tgas):     self.cyl2d_tgas    = np.zeros((nr,nz))
        if(rhodust):  self.cyl2d_rhodust = np.zeros((ndust,nr,nz))
        if(irrad):
        ...
        if(visc):     self.cyl2d_visc_src = np.zeros((nr,nz))
        if(ross):     self.cyl2d_meanopacross = np.zeros((nr,nz))
        for ir in range(nr):
            if(rhogas):   self.cyl2d_rhogas[ir,:]  = self.verts[ir].rhogas.copy()
            if(tgas):     self.cyl2d_tgas[ir,:]    = self.verts[ir].tgas.copy()
            if(rhodust):
            ...
            if(visc):     self.cyl2d_visc_src[ir,:] = self.verts[ir].visc_src.copy()
            if(ross):     self.cyl2d_meanopacross[ir,:] = self.verts[ir].mean_opacity_rosseland.copy()
    ...
    def convert_cyl2d_to_1p1d(self,rhogas=False,tgas=False,rhodust=False,selfgrav=False):
        ...
        if hasattr(self.disk, 'dust'):
            ndust       = len(self.disk.dust)
        for ir in range(nr):
            if(rhogas):   self.verts[ir].rhogas[:]   = self.cyl2d_rhogas[ir,:]
            if(tgas):     self.verts[ir].tgas[:]     = self.cyl2d_tgas[ir,:]
            if(rhodust):
                for idust in range(ndust):
    ...
    def coord_trafo_cyl2d_to_spher2d(self,cyl2d_q,fill_value=np.nan,midplanemargin=False):
        ...
        """
        if midplanemargin: self.spher_tt[:,-1] = nc.pi/2 * ( 1.0 - 1e-9 )
    ...
    def solve_2d_rad_diffusion(self, floorparam=1e-6, thist=False, limiter=None,
        ...
        #
        if not hasattr(self, 'cyl2d_irrad_src'): self.cyl2d_irrad_src = 0.
        if not hasattr(self, 'cyl2d_visc_src'): self.cyl2d_visc_src = 0.
    ...
    def solve_2d_selfgrav(self,incl_gas=True,incl_dust=True,cellwalls=False,
        ...
        if incl_gas:  self.cyl2d_rho += self.cyl2d_rhogas
    ...
    def solve_selfgrav_spher(self,rho,convcrit=1e-10,itermax=10000):
        ...
        if self.spher_theta.max() < 0.9 * np.pi: mass *= 2
    ...
    def laplace_sper_2d(self,pot):
        ...
        laplace[1:-1,1:-1] += (1./rr[1:-1,1:-1]**2)                                      \
             * ((rrr**2)*(pot[2:,1:-1]   - pot[1:-1,1:-1])/(rr[2:,1:-1]-rr[1:-1,1:-1])   \
               -(rrl**2)*(pot[1:-1,1:-1] - pot[:-2,1:-1])/(rr[1:-1,1:-1]-rr[:-2,1:-1]))  \
                / (rrr-rrl)
        laplace[1:-1,1:-1] += (1./(rr[1:-1,1:-1]**2*np.sin(tt[1:-1,1:-1])))              \
             * (sinttr*(pot[1:-1,2:]   - pot[1:-1,1:-1])/(tt[1:-1,2:]-tt[1:-1,1:-1])     \
